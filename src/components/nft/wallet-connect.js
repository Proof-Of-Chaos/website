...
export default function WalletConnect({
  ...
  const connectedAccounts = useAppStore(
    (state) => state.user.connectedAccounts
  );
  ...
  const updateConnectedAccounts = useAppStore(
    (state) => state.updateConnectedAccounts
  );
  const updateConnectedAccount = useAppStore(
    (state) => state.updateConnectedAccount
  );
  ...
  if (isMounted && window.walletExtension?.isNovaWallet) {
    walletList = [new PolkadotjsWallet(), new SubWallet(), new NovaWallet()];
  }
        ...
        <WalletSelect
          ...
          onAccountSelected={(newAccount) => {
            ...
            if (connectedAccounts?.length) {
              const connectedAccountIndex = connectedAccounts.findIndex(
            ...
            >
              {connectedAccount ? (
                <>
                  ...
                  <span className="pl-0 sm:pl-3 hidden sm:block">
