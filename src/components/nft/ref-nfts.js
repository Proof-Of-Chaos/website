...
import { microToKSMFormatted } from "../../utils/utils";
...
const isOwned = (ref, userNFTs, symbol, rarity) => {
  ...
  if (refIndex > 192) {
  ...
  } else if ([188, 190, 192].includes(refIndex)) {
    // thumb based

    const referendumConfig = websiteConfig.classic_referendums.find(
      (referendum) => referendum.ref === ref && referendum.rarity === rarity
    );
    ...
    return referendumConfig.thumbs
      ? referendumConfig.thumbs.some((r) => userNFTResources.includes(r))
      : false;
  } else {
    // resource based (deprecated)

    const userNFTMetadata = userNFTs?.map(({ metadata }) => metadata) ?? [];
    const referendumConfig = websiteConfig.classic_referendums.find(
      (referendum) => referendum.ref === ref && referendum.rarity === rarity
    );
    return referendumConfig.resources
      ? referendumConfig.resources.some((r) => userNFTMetadata.includes(r))
      : false;
  }
};
...
const NFTScore = ({ score }) => {
  return (
    <Tippy content="The leaderboard score boost you will get when you equip this NFT onto your shelf">
      <span
        className={`absolute z-10 -mt-3 px-2 -mr-2 right-0 nft-score cursor-default`}
      >
...
const FloorPrice = ({ nfts, url }) => {
  const sortedFloorNFTs = nfts?.sort(
    (a, b) =>
      parseFloat(
        a.forsale /
          (1 - a.properties.royaltyInfo.value.royaltyPercentFloat / 100)
      ) -
      parseFloat(
        b.forsale /
          (1 - b.properties.royaltyInfo.value.royaltyPercentFloat / 100)
      )
  );
  const floorNFT = sortedFloorNFTs[0];
  const forsale = floorNFT?.forsale;
  const royaltyPercentFloat =
    floorNFT?.properties?.royaltyInfo?.value?.royaltyPercentFloat;
  const realPrice =
    forsale / (1 - websiteConfig.singular_fee - royaltyPercentFloat / 100);
  return (
    <Tippy content="The floor price of this NFT on singular.app">
      <span
        className={`absolute z-10 cursor-default px-2 -mr-2 right-0 bottom-0 -mb-2 nft-floor no-underline`}
      >
        <FontAwesomeIcon icon={faBroom} size={"sm"} />{" "}
        {formatBalance(realPrice, {
          decimals: 12,
          forceUnit: "-",
          withSi: true,
          withUnit: "KSM",
  ...
  return (
    <div className="single-nft relative p-4 transform transition duration-200 hover:scale-105 flex justify-center flex-col items-center">
      <div className="relative mb-4">
        ...
        {isMounted &&
          showFloor &&
          parseInt(ref.split(" ")[1]) > 191 &&
          floorNFTs?.nfts?.length > 0 && (
            <FloorPrice nfts={floorNFTs.nfts} url={url} />
          )}
        <span className={`absolute z-10 -ml-4 -mt-3 px-2 nft-${rarity}`}>
          {rarity}
        </span>
        {thumb && thumb !== "" ? (
          <Image
...
export default function NFTDetail({ nfts, scores }) {
  ...
  return (
    <div className="nft-detail mx-4 mb-4 p-6 pb-10 border-b-2 transition-shadow duration-200 dark:bg-light-dark">
      ...
      {allDescription && (
        <p className="pb-5 max-w-3xl">
          <ReactMarkdown>{allDescription}</ReactMarkdown>
        </p>
      )}
      <div className="flex flex-wrap justify-between">
        {["common", "rare", "epic", "legendary"].map((rarity, idx) => {
          let nftByRarity = distinctUserNFTs.find(
        ...
        >
          ...
          >
            <Button className="border-2 text-gray-800 w-full no-underline mt-4">
              Get on
              <svg
                className="pl-3 h-5"
