...
function SingleNFT( { id, rarity, data: { thumb, artist, amount } } ) {
  return (
    <div className="single-nft rounded-lg p-4">
...
export default function NFTDetail( { nft } ) {
  const { id } = nft;
  return (
    <div className="nft-detail mb-3 hover:shadow-lg shadow-sm flex justify-between rounded-lg bg-white p-5 transition-shadow duration-200 dark:bg-light-dark xs:p-6">
      <div className="flex flex-col justify-between">
        ...
        <Button className="bg-brand-600 bg-opacity-25" variant="calm">
          Get on Singular
        </Button>
      </div>
      { [ 'common', 'rare', 'epic' ].map( rarity => (
        <div key={ `${id}-${rarity} `} className={ `nft-detail-${rarity} pr-2` }>
          <SingleNFT
            id={ id }
            data={ nft[ rarity ] }
            rarity={ rarity }
