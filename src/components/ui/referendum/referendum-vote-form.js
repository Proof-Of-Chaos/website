...
export function ReferendumVoteForm({ referendumId }) {
  ...
  const voteAmountLabel = isBalanceLoading ? (
    <div className="text-sm font-normal text-right">
      {" "}
      (available: <InlineLoader /> ){" "}
    </div>
  ) : (
    <span className="text-sm font-normal text-right float-right">{`(available: ${availableBalance?.toFixed(
      2
    )} KSM)`}</span>
  );
              ...
              {...register("vote-amount-aye", {
                validate: {
                  ...
                  hasEnoughFunds: (value) =>
                    availableBalance && parseFloat(value) <= availableBalance,
                },
              })}
            />
          ...
          <>
            ...
            <div className="mx-3">
              <Slider
                ...
                className={classNames("conviction-slider")}
                defaultValue={1}
                min={0}
                max={6}
                marks={marks}
