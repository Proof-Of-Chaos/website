  ...
  useEffect(() => {
    if (gov2refs && gov2refs.length) {
      ...
      if (userVotes && userVotes.length) {
        ...
        const userVotesForActive = userVotes.filter((vote) =>
          activeRefIndices.includes(vote.referendumIndex)
        );
        voteCounts = {
      ...
      <div className="filters">
        {tracks &&
          tracks.map((track, idx) => {
            const trackInfo = KUSAMA_TRACK_INFO.find(
              (t) => t.id === parseInt(track[0])
            );
            return (
        ...
        {tracks && counts.voted && (
          <>
            ...
            <Tippy content={"Only show referenda, you already voted on"}>
              ...
              >
                {votedFilterText}{" "}
                <span className="text-xs text-slate-500">{counts.voted}</span>
              </button>
            </Tippy>
          </>
        )}
        ...
        {filteredRefs.map((r) => (
          <ReferendumDetail
            ...
            userVote={userVotes?.find(
              (vote) => vote.referendumIndex === r.index
            )}
          />
        ))}
      </ul>
