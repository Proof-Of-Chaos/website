...
import { microToKSM, getEndDateByBlock } from "../utils/utils";
  ...
  return new Promise(async (resolve) => {
    ...
    if (referendumIndex === null || isNaN(referendumIndex)) {
      ...
      if (quizData?.quizzes?.length > 0) {
        latestQuiz = quizData.quizzes.sort(
          (a, b) => parseInt(b.version) - parseInt(a.version)
        )[0];
      }

      resolve({
        ...result.data.referendaStats.find(
          (el) => el.index === referendumIndex
        ),
...
export const pastReferendumFetcher = async () => {
  return new Promise(async (resolve) => {
    ...
    let pastReferendaStats20 = result.data.referendaStats.filter((ref) =>
      pastReferendums20.includes(ref.index)
    );
    ...
    pastReferendaStats20 = pastReferendaStats20.map((ref, idx) => {
      return {
        ...ref,
        title: pastReferendaPAData20.find(
          (el) => el.onchain_link.onchain_referendum_id === ref.index
        )?.title,
        description: pastReferendaPAData20.find(
          (el) => el.onchain_link.onchain_referendum_id === ref.index
        )?.content,
      };
    });

    const sortedReferendaStats = pastReferendaStats20.sort(
...
export const activeReferendumFetcher = async (ksmAddress) => {
  ...
  for (const referendum of activeReferendums) {
    const endDate = await getEndDateByBlock(
      referendum.status.end,
      number,
      timestamp
    );
    const PAData = await getPADataForRefs([referendum.index.toString()]);
    const PADatum = PAData?.[0];
    const quizData = quizzesData.find(
      (q) => q.referendumIndex === referendum.index
    );
...
const referendumObject = (
  ...
  if (!title && referendum.image) {
    title =
      referendum.image.proposal.section.toString() +
      "." +
      referendum.image.proposal.method.toString();
  }
  //getlatestversion
  let latestQuiz = quizData?.sort(
    (a, b) => parseInt(b.version) - parseInt(a.version)
  )[0];
  let allSubmissions = [];
  ...
  return {
    ...
    voted_amount_total: microToKSM(
      parseFloat(referendum.votedNay) + parseFloat(referendum.votedAye)
    ),
    ended_at: null,
...
export const usePastReferendums = () => {
  return useQuery(["pastReferendumData"], async () => {
    return pastReferendumFetcher();
  });
};

export const useReferendum = (referendumIndex) => {
  return useQuery(["referendum", referendumIndex], async () =>
    singleReferendumFetcher(referendumIndex)
  );
};

export const useEndDate = (endBlock) => {
  return useQuery(["referendumEndDate", endBlock], async () => {
    const api = await getApiKusama();
    const { hash, number } = await api.rpc.chain.getHeader();
    const timestamp = await api.query.timestamp.now.at(hash);
